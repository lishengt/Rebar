classdef PSO
    ...
    methods
        ...
        function [x, best_x] = iter(obj, x)
        
            is_nan = all(all(isnan(x)));
        
            if is_nan
                x = randi([0, 1], obj.n_individuals, 3*obj.n_vars);
            end
        ...
        function is_ok = applyPrevious(obj, x)
            is_ok = 1;
            forward = ones(8, obj.n_vars);
            
            x_res = obj.problem.solve(obj.decode(x));
            
            res = zeros(1,5);
            
            for i = 1:8
               u = forward(i,:) * (i-1);
               u = repmat(u, obj.n_individuals, 1);
               res(i) = mode(obj.problem.solve(u)); 
            end
        
            if mode(x_res) == min(res)
               is_ok = 0; 
            end      
        end
    end
